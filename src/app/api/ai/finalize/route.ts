...
export async function POST(req: NextRequest) {
  try {
    ...
    if (attempt.status !== 'recording_uploaded') {
      return NextResponse.json(
        { error: `Cannot finalize: status is ${attempt.status}, expected recording_uploaded` },
        { status: 400 }
      );
    }
    ...
    if (!attempt.recording_url) {
      return NextResponse.json(
        { error: 'Cannot finalize: recording_url is missing' },
        { status: 400 }
      );
    }
